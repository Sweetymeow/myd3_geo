...
d3.csv("netsale.csv", function(dataset){ 
       ...
       // console.log(statusData[j]); 
        ...
        dashrect.append("rect")
            .attr("x", function(d, i) {
                ...
                if((d.date%3==1 || d.total)*j){
                    quartLineX[m] = i?(x0-3):x0;
                    m++;
                }
                return i?(x0-3):x0;
            })
    ...
    for(var x in quartLineX){
        console.log(quartLineX[x]);
        svg.append("g")
            .attr("class", "quartLine")
            .append("line")
            .attr("x1",0)
            .attr("y1",0)
            .attr("x2",0)
            .attr("y2",h+barRise)
            .attr("transform", "translate(" + quartLineX[x] + ", 0)")
